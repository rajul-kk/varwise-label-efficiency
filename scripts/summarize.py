...
def section_curves(track):
    p = RDIR / f"curves_track_{track}.csv"
    if not p.exists():
        return None
    ...
    print(f"\n### Track {track.upper()} — final scores at {budget} labels\n")
    ...
    print(f"\n### Track {track.upper()} — per-class F1 at {budget} labels, "
...
def section_primary(track):
    p = RDIR / f"primary_savings_track_{track}.csv"
    if not p.exists():
        return
    prim = pd.read_csv(p)
    print(f"\n### Track {track.upper()} — labels to match random sampling's "
          "final score\n")
...
def main():
    ...
    for track in ("b", "a"):
        section_curves(track)
        section_primary(track)
