...
| 2b | A third, independent line of evidence for the same YSO/`agn` confusion, via Gaia parallax rather than photometry or cross-classification: SIMBAD-discordant `agn` show a 100–150× elevated significant-parallax rate vs SIMBAD-confirmed `agn`, concentrated in YSO/pre-main-sequence SIMBAD types | **Novel — new physical method, same underlying finding as #2** | No prior astrometric validation of VarWISE found. Strengthens #2 by triangulation (different physics, same conclusion) rather than standing as an independent discovery. The *aggregate* `agn` parallax result is mostly reassuring for VarWISE — worth reporting alongside the contamination finding, not overshadowed by it. |
